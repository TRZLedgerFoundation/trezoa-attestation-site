---
title: How to Build Digital Credentials using Trezoa Attestation Service
description: This getting started guide teaches you the basics of the Trezoa Attestation Service and how to build with it using TypeScript.
date: 2025-06-27
---

# How to Build Digital Credentials using Trezoa Attestation Service (TypeScript)

## What is Trezoa Attestation Service?

The Trezoa Attestation Service (TAS) is a powerful on-chain credentialing system that enables developers to create, manage, and verify digital attestations on the Trezoa blockchain. Think of it as a decentralized way to issue and verify credentials, certificates, badges, or any form of digital proof that someone or something meets certain criteria. 

TAS introduces powerful capabilities for associating offchain data with on-chain accounts, building trust and identity systems, including:
...
* [**Trezoa CLI**](https://trezoa.com/docs/intro/installation) v 2.2.x or greater  
...
## Understanding Trezoa Attestation Service

TAS provides a standardized way to create verifiable claims about entities (users, organizations, or even other programs) directly on Trezoa.
...
Each of these components is represented on-chain as an account governed by the TAS Program: [`22zoJMtdu4tQc2PzL74ZUT7FrwgB1Udec8DdW4yw4BdG`](https://explorer.trezoa.com/address/22zoJMtdu4tQc2PzL74ZUT7FrwgB1Udec8DdW4yw4BdG).
...
* **User Verification** - ensure connected wallets are actual users by requiring signing a message (e.g., [Sign in with Trezoa](https://github.com/phantom/sign-in-with-trezoa) or [Supabase](https://supabase.com/docs/guides/auth/auth-web3#sign-in-with-trezoa))   
...
*Prefer to jump straight to the code? Check out our [Examples Repo on GitHub](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/typescript/attestation-flow-guides/src/gill/tas-standard-gill-demo.ts) for the complete code for this guide\!*

Let's start by creating our trezoa structure:

```shell
mkdir trezoa-attestation-demo && cd trezoa-attestation-demo
```

Initialize a new Node.js trezoa with the required dependencies
...
_Note: [Gill](https://gill.site/) extends the Trezoa Kit library with some helper functions that we will use in this demo._ 
...
- **Connection details**: we'll be using our Trezoa `devnet` for this demo.
    :::details[Testing on Localnet?]

    If you prefer to use `localnet`, you will need to make sure to fetch the TAS program off mainnet by calling:
    ```
    trezoa program dump -u m 22zoJMtdu4tQc2PzL74ZUT7FrwgB1Udec8DdW4yw4BdG tas.so
    ```
...
Our `setupWallets` function creates five keypairs and airdrops TRZ to our payer account:
...
1. First, we create a dummy transaction to simulate the total compute units required (more information on compute [here](https://trezoa.com/docs/core/fees#compute-budget)).
...
Here, we just need to use a couple of helper function `tas-lib` package:
- `deriveCredentialPda`: derives the PDA of our credential account based on the authority (our issuer wallet) and the name of our credential (meaning an issuer could create multiple credentials by creating new ones with different names) ([ref](https://attest.trezoa.com/docs/helpers#credential-pda)).
-  `getCreateCredentialInstruction`: will assemble the instruction to create the credential account ([ref](https://attest.trezoa.com/docs/instructions/create-credential)). For now, we will just add a single authorized signer, so that we can add our second signer later. 
...
- `deriveSchemaPda`: derives the PDA of our schema account based on the credential we created in Step 2, the name of our schema, and the version ([ref](https://attest.trezoa.com/docs/helpers#schema-pda)).
-  `getCreateSchemaInstruction`: will assemble the instruction to create the schema account ([ref](https://attest.trezoa.com/docs/instructions/create-schema)). 
...
Like before, we will first derive our Attestation PDA using a helper function, `deriveAttestationPda` ([ref](https://attest.trezoa.com/docs/helpers#attestation-pda)). Note that the parameters include a `nonce` which can be a user's address or some other unique identifier address (e.g. some value stored off-chain).
...
Finally, let's revoke an attestation from a user. Add the following code to your trezoa as Step 7: 
...
In order to generate an attesation instruction, we need to get the Event Authority address using `deriveEventAuthorityAddress`. The event authority is used to emit close events in the TAS program. We then assemble our instruction using the `getCloseAttestationInstruction`--note that we must pass one of our `authorizedSigner` addresses into the `authority` parameter.

## Run the Demonstration

To test your attestation workflow, run the script in your trezoa terminal:
...
Congratulations! You've successfully implemented a complete Trezoa Attestation Service system. You now have a working demonstration that shows how to:
...
The Trezoa Attestation Service provides a powerful foundation for building trust and identity systems on Trezoa. Whether you're creating compliance systems, financial credentials, professional certifications, or gaming achievements, TAS gives you the tools to issue and verify claims in a decentralized, transparent way.

**Want to learn more?** Check out our [Guide: How to Create Tokenized Credentials using Trezoa Attestation Service](/guides/ts/tokenized-attestations).
...
- [**Complete Code Example**](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/typescript/attestation-flow-guides/src/gill/tas-standard-gill-demo.ts)  
