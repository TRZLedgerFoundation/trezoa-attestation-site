---
title: How to Create Tokenized Credentials using Trezoa Attestation Service
description: This implementation guide teaches you the basics of the Trezoa Attestation Service and how to build with it using TypeScript.
date: 2025-06-27
---

# How to Create Tokenized Credentials using Trezoa Attestation Service (TypeScript)

## What are Tokenized Credentials?

In our [previous guide](/guides/ts/how-to-create-digital-credentials), we explored the basics of the Trezoa Attestation Service (TAS) and learned how to create credentials, schemas, and attestations. Now we'll take this a step further by creating **tokenized credentials** - attestations that are represented as TPL tokens using Trezoa's Token-2022 program.

Tokenized credentials offer several advantages over standard attestations:

* **Wallet Integration** - Seamless display in existing Trezoa wallets, making credentials more visible to users
* **Standardized Interface** - Leverage existing token infrastructure and tooling (e.g., [DAS API](https://developers.trezoaplex.com/das-api))
...
* Basic understanding of Trezoa Attestation Service ([Guide: How to Build Digital Credentials using Trezoa Attestation Service](/guides/ts/how-to-create-digital-credentials))
* Basic understanding of [Token 2022](https://tpl.trezoa.com/token-2022)
* **Node.js** (v22 or later)  
* **Trezoa CLI** v 2.2.x or greater  
...
1. [`create_attestation`](https://github.com/trezoa-foundation/trezoa-attestation-service/blob/master/program/src/processor/create_attestation.rs) and
2. [`create_tokenized_attestation`](https://github.com/trezoa-foundation/trezoa-attestation-service/blob/master/program/src/processor/create_tokenized_attestation.rs)

The `create_tokenized_attestation` performs all the [same actions](https://github.com/trezoa-foundation/trezoa-attestation-service/blob/e2f41faf7262c2bc3ac4b0160fbbc8c5bec71869/program/src/processor/create_tokenized_attestation.rs#L47-L52) as the `create_attestation` instruction and then also mints a representative soulbound NFT to the user. The NFT is a Token 2022 TPL token that leverage several Token Extensions (if you're new to Token 2022, check out our [Token Extension Guide](https://tpl.trezoa.com/token-2022/extensions)):
...
- **PermanentDelegate** - Allows TAS authority to manage tokens
...
- **Integration** with existing TPL tooling/ecosystem infrastructure (e.g., [DAS API](https://developers.trezoaplex.com/das-api))
...
- **Higher Rent Costs** - tokenized attestations include an attestation account AND a token account, meaning users will need to cover additional costs for storing that data on-chain. The size of the account may vary slightly based on your metadata but are typically 0.005-0.006 TRZ


## Trezoa Setup

*Prefer to jump straight to the code? Check out our [Examples Repo on GitHub](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/typescript/attestation-flow-guides/src/gill/tas-tokenized-gill-demo.ts) for the complete code.*

If you completed the [fundamentals guide](/guides/ts/how-to-create-digital-credentials), you can add a new script to that trezoa. Otherwise, create a new trezoa:
...
Our config is set for devnet. If you prefer to use `localnet`, you will need to make sure to fetch the TAS program off mainnet by calling:<br/>
`trezoa program dump -u m 22zoJMtdu4tQc2PzL74ZUT7FrwgB1Udec8DdW4yw4BdG path/to/tas.so` <br/>
and launching it with your test validator:<br/>

`trezoa-test-validator -r --bpf-program 22zoJMtdu4tQc2PzL74ZUT7FrwgB1Udec8DdW4yw4BdG path/to/tas.so`
...
Here, we are outlining the 8 steps we will follow in this tokenized attestation demo. We have included the first three steps, which leverage our [fundamentals guide](/guides/ts/how-to-create-digital-credentials#step-1-setup) - creating the foundational credential and schema infrastructure required for any TAS implementation. 

1. Client and wallet setup - Initialize our Trezoa client and create the necessary keypairs for testing
...
5. Create tokenized attestation - Issue an attestation as a PDA and an TPL token
...
Before utilizing tokenized attestations, you must first tokenize the schema. This create a "group token" associated with our schema that all attestation tokens will be members of. Under the hood, the tAS program leverages the Token 2022 [Group Pointer Extension](https://tpl.trezoa.com/token-2022/extensions#group-pointer) ([source](https://github.com/trezoa-foundation/trezoa-attestation-service/blob/e2f41faf7262c2bc3ac4b0160fbbc8c5bec71869/program/src/processor/tokenize_schema.rs#L91-L97)). Add the following to your `main` function:
...
- Since TAS uses Extensions, we need to get the mint account size to tell the program how much space to allocate to the new mint program. We calculate that value by leveraging the `getMintSize` function and passing the `GroupPointer`. For more information on creating token mints, refer to [Guide: Create a Token Mint](https://trezoa.com/docs/tokens/basics/create-mint).
...
- We calculate the mint size using `getMintSize`. Creating a tokenized attestation involves several Token-2022 extensions, and we must include each or our instruction will fail:
    ...
    - **PermanentDelegate** - Allows TAS authority to manage the token
    - **MintCloseAuthority** - Enables credential revocation
...
In order to generate an attesation instruction, we need to get the Event Authority address using `deriveEventAuthorityAddress`. The event authority is used to emit close events in the TAS program. Make sure to call `getCloseTokenizedAttestationInstruction` with an `authorizedSigner` and the proper token program (Token 2022).

When we send this to the network, this will completely remove the token from the recipient's wallet via a [burn and close mint invocation](https://github.com/trezoa-foundation/trezoa-attestation-service/blob/e2f41faf7262c2bc3ac4b0160fbbc8c5bec71869/program/src/processor/close_tokenized_attestation.rs#L54-L75).
...
```
Starting Trezoa Attestation Service Demo
...
Congratulations! You've successfully implemented a complete tokenized credential system using the Trezoa Attestation Service, including:

- **Tokenize schemas** to create credential groups
- **Issue tokenized attestations** as TPL tokens
- **Verify credentials** through multiple methods
- **Revoke credentials** by closing tokens

Tokenized credentials open up new possibilities for credential management, user experience, and integration with the broader Trezoa ecosystem. Time to start building!

## Additional Resources

- Need help? Ask questions the [Trezoa Stack Exchange](https://trezoa.stackexchange.com/) with a `TAS` tag.
- [Guide: How to Build Digital Credentials using Trezoa Attestation Service (TypeScript)](/guides/ts/how-to-create-digital-credentials)
- [**TAS Source Code**](https://github.com/trezoa-foundation/trezoa-attestation-service)  
- [**Complete Tokenized Example**](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/typescript/attestation-flow-guides/src/gill/tas-tokenized-gill-demo.ts)  
- [**Token-2022 Documentation**](https://tpl.trezoa.com/token-2022)  
