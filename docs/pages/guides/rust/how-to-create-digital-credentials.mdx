...
description: This getting started guide teaches you the basics of the Trezoa Attestation Service and how to build with it using Rust.
date: 2025-7-9
---

# How to Build Digital Credentials using Trezoa Attestation Service (Rust)
...
TAS enables robust connections between off-chain data and on-chain accounts, supporting the development of trusted identity and reputation systems, including:
...
*Prefer to jump straight to the code? Check out our [Examples Repo on GitHub](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/rust/attestation-flow-guide/standard-demo) for the complete code for this guide!*
...
[dependencies]
# Trezoa core dependencies
trezoa-sdk = "2.3.1"
trezoa-client = "2.3.2"
trezoa-program = "2.3.0"
trezoa-system-interface = "1.0.0"

# Trezoa Attestation Service client
...
Here we are just setting up some basic configuration for our attestation test. First we define a config that includes an RPC (in this case we are testing on localhost using trezoa test validator), some information about our **Schema** and the duration of our attestation.
...
- The `schema_layout` vec defines the layout of the data stored based on specific data type mappings from the Trezoa Attestation Service's type system ([ref: type map chart](/schemas#schema-layout-data-types)). In our example, `vec![12, 0, 12]` maps to our three fields: 
...
All we are doing here is specifying that our demo utilizes the *Config* defined in the previous step, setup the wallets needed for the demo, and create a Trezoa client with the specific RPC.  
...
This method funds the payer account with 1 TRZ to cover our transaction fees.
...
- The `trezoa-attestation-service-client` crate includes a helpful instruction builder for each of our program's instructions. Here we call `CreateCredentialBuilder::new()` to create a credential account that serves as the issuing authority, with one initial authorized signer, `authorized_signer1`. 
...
What if we want to change the list of authorized signers for a credential? The TAS Program enables that. Let's add functionality to change our authorized signers:
...
Before running the demo, you'll need to set up a local Trezoa validator with the TAS program loaded:

1. **Fetch the TAS program** (required if running local test validator):
```bash
...
Congratulations! You've successfully implemented a complete Trezoa Attestation Service system in Rust. You can expand on this to build your own client for creating or verifying Trezoa Attestations. 
...
**Want to learn more?** Check out our [Guide: How to Create Tokenized Credentials using Trezoa Attestation Service](/guides/rust/tokenized-attestations).


## Additional Resources

- Need help? Ask questions on the [Trezoa Stack Exchange](https://trezoa.stackexchange.com/) with a `TAS` tag.
- [**TAS Source Code**](https://github.com/trezoa-foundation/trezoa-attestation-service)  
- [**Complete Rust Example**](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/rust/attestation-flow-guide/standard-demo)  
