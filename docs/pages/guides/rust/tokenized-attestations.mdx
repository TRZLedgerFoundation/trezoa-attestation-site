...
description: This implementation guide teaches you the basics of the Trezoa Attestation Service and how to build with it using Rust.
date: 2025-7-9
---
# How to Build Tokenized Digital Credentials using Trezoa Attestation Service (Rust)

## Introduction

This guide builds upon our [Rust TAS Flow Guide](/guides/rust/how-to-create-digital-credentials) where you learned the fundamentals of creating credentials, schemas, and attestations using the Trezoa Attestation Service. In this Part 2, we'll extend your existing project to create **tokenized attestations** - NFTs that represent your digital credentials.
...
This guide will walk you through creating, verifying, and closing tokenized attestations using the `trezoa-attestation-service-client` crate.

## Prerequisites

Before starting this tutorial, ensure you have:

* Completed [Part 1](/guides/rust/how-to-create-digital-credentials) - This guide builds directly on that foundation (or you can fork our existing code from [GitHub](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/rust/attestation-flow-guide/standard-demo))
...
Tokenized attestations leverage TPL Token 2022's advanced features to create secure, non-transferable NFTs that represent credentials. Here's how the system works:
...
* **Token Extensions** - The TAS program utilizes several TPL Token 2022 extensions: 
    ...
    * **Permanent Delegate** - TAS program maintains control over tokens
    * **Mint Close Authority** - TAS program maintains ability to burn the token mint

## Project Extension

This project builds on the *SasDemo* struct from the [TAS Rust Workflow Guide](/guides/rust/how-to-create-digital-credentials). You will need to start with that existing code or fork it from the [GitHub Examples Repository](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/rust/attestation-flow-guide/standard-demo).

### Update Dependencies

First, add the TPL Token 2022 dependencies to your existing `Cargo.toml`:
...
[dependencies]
# Trezoa core dependencies (existing)
trezoa-sdk = "2.3.1"
trezoa-client = "2.3.2"
trezoa-program = "2.3.0"
trezoa-system-interface = "1.0.0"

# TPL Token Program dependencies (NEW)
tpl-token-2022 = "9.0.0"
tpl-token-metadata-interface = "0.7.0"
tpl-associated-token-account = "7.0.0"
tpl-pod = "0.5.1"
tpl-token-group-interface = "0.6.0"

# Trezoa Attestation Service client (existing)
trezoa-attestation-service-client = "1.0.9"
...
```rust
use tpl_pod::optional_keys::OptionalNonZeroPubkey;
use tpl_token_2022::{
    extension::{BaseStateWithExtensions, ExtensionType, StateWithExtensions},
    state::Mint,
    ID as TOKEN_22_PROGRAM_ID,
};
use tpl_token_metadata_interface::state::TokenMetadata;
use tpl_associated_token_account::get_associated_token_address_with_program_id;
use tpl_token_group_interface::state::TokenGroupMember;
```

And add these additional instruction builders to your `trezoa_attestation_service_client` imports:
...
impl SasDemo {
    ...
    fn derive_sas_authority_address() -> (Pubkey, u8) {
        Pubkey::find_program_address(&[b"tas"], &TREZOA_ATTESTATION_SERVICE_ID)
    }
...
- **TAS Authority** - The program's authority account that manages tokens

### Mint Account Size Calculation

Because TPL Token 2022 token mints vary in size based on the extensions enabled, the Token 2022 program requires precise account size calculations when creating new tokens. Add these helper methods to size our schema and attestation token mints:
...
This method follows the same pattern that we used in our previous guide: derive PDAs, assemble an instruction with the TAS Client instruction builders (*note the authority here is the `issuer`*), and then send the instruction to the network. 
...
- Creating the tokenized attestation -- note that under the hood, the TAS program will initialize both a tokenized attestation AND the standard attestation PDA ([source code](https://github.com/trezoa-foundation/trezoa-attestation-service/blob/e2f41faf7262c2bc3ac4b0160fbbc8c5bec71869/program/src/processor/create_tokenized_attestation.rs#L47-L52))--we will verify both
...
**Fetch the TAS program** (if you don't already have it):
```bash
trezoa program dump -u m 22zoJMtdu4tQc2PzL74ZUT7FrwgB1Udec8DdW4yw4BdG path/to/tas.so
```
...
```
Starting Trezoa Attestation Service Tokenized Demo
...
Congratulations! You've successfully extended your Trezoa Attestation Service implementation with tokenization capabilities. Your TAS workflow now enables you to create NFTs that represent digital credentials, offering enhanced user experiences and wallet integration: 
...
- **[Guide: How to Build Digital Credentials using Trezoa Attestation Service (Rust)](/guides/rust/how-to-create-digital-credentials)** - Foundation concepts
- **[Complete Tokenized Example](https://github.com/trezoa-foundation/trezoa-attestation-service/tree/master/examples/rust)** - Full source code
